...
Prevent org-roam from handling JIRA ids as node ids.
#+begin_src emacs-lisp
(defun ma/is-jira-node (&optional pom create prefix)
  (org-with-point-at pom
    (not (org-entry-get nil "CUSTOM_ID"))))

(require 'org-id)
(advice-add 'org-id-get :before-while 'ma/is-jira-node)
#+end_src
...
(defun ma/agenda-issue-name (item)
  (-when-let* ((marker (org-super-agenda--get-marker item))
               (title (->> marker marker-buffer buffer-name ma/org-file-get-title)))
    (concat "Issue: " title)))
