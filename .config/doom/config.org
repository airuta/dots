...
(setq ;; System locations
      ...
      org-shopping-file      (concat org-directory "/shopping.org")
      ;; Dirs
...
#+begin_src emacs-lisp
(defun ma/org-roam-capture-daily-to-heading (key category entry file-format heading &optional JUMP FINISH PLAIN &key goto)
...
(defun ma/org-roam-capture-daily (entry heading &optional JUMP FINISH PLAIN &key goto)
...
(map! :prefix ("C-c n" . "notes")
      ...
      :desc "today"   :g "d" (cmd! (ma/org-roam-capture-daily "" "Journal" :goto t)))
#+end_src
...
(defun ma/refresh-agenda-files ()
  ...
  (setq org-agenda-files
        (append
         (list org-habits-file org-events-file org-shopping-file)
...
(defun ma/clock-out-handler ()
  (let* ((start-time (format-time-string "%H:%M" org-clock-start-time))
    ...
    (ma/org-roam-capture-daily entry "Journal" nil t t)
    ))
